...
// Penghapusan berhenti sendiri saat teks perintahnya habis.
//
// Di sebelah kirinya ada PROMPT, dan prompt tidak akan pernah digambar ulang —
// shell hanya menggambar ulang buffer-nya. Satu penghapusan yang kelewat
// berarti "❯" lenyap dan tidak kembali sampai perintah berikutnya dijalankan.
func TestEchoBackspaceTidakMenembusPrompt(t *testing.T) {
	var buf bytes.Buffer
	r := NewRenderer(&buf, 80, 24, true)
	r.Milik(2) // shell menggambar dua karakter, misalnya "cd"

	for i := 0; i < 5; i++ {
		if err := r.EchoBackspace(); err != nil {
			t.Fatal(err)
		}
	}
	if got := strings.Count(buf.String(), "\b \b"); got != 2 {
		t.Errorf("menghapus %d kali, mau berhenti di 2 — sisanya prompt", got)
	}
}

// Karakter yang digemakan SESI boleh dihapus lebih dulu, baru teks milik shell.
func TestEchoBackspaceMenghabiskanGemaLebihDulu(t *testing.T) {
	var buf bytes.Buffer
	r := NewRenderer(&buf, 80, 24, true)
	r.Milik(1)

	if err := r.EchoRune('x'); err != nil { // gema sesi: 1
		t.Fatal(err)
	}
	buf.Reset()
	...
	// Satu gema sesi + satu milik shell = dua; sisanya ditolak.
	if got := strings.Count(buf.String(), "\b \b"); got != 2 {
		t.Errorf("menghapus %d kali, mau 2", got)
	}
}

// Tanpa Milik, tidak ada yang boleh dihapus selain gema sesi sendiri: renderer
// yang tidak diberi tahu batasnya harus memilih yang aman.
func TestEchoBackspaceTanpaMilikTidakMenghapusApaPun(t *testing.T) {
	var buf bytes.Buffer
	r := NewRenderer(&buf, 80, 24, true)
	...
	if buf.Len() != 0 {
		t.Errorf("menulis %q, mau tidak menulis apa pun", buf.String())
	}
}
